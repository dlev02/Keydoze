...
- **Distribution signing — 6 September 2026:** A separate optimized Apple-silicon app was signed with Developer ID Application for team `M379WPVT8A`, hardened runtime, and a secure timestamp. Strict signature verification passed. Apple received notarization submission `1b3d5b85-951a-4049-98cf-ab8d2cf72ff4` for version 0.1.0 build 2 on 6 September 2026 (local time); its latest observed status is **In Progress**. Notarization acceptance, ticket stapling, Gatekeeper acceptance, and a public binary download are still pending. The stable development app was preserved.
...
The title and toolbar background are hidden while standard window controls remain. Content uses a continuous app surface. Build 2 replaces the scrolling standard About panel with a compact SwiftUI window showing the app icon, version, offline privacy summary, source and Apache 2.0 links, and Drew Levinson credit. Its 340 × 328-point content was checked in light and dark AppKit renders, including native link controls. The optimized build and strict signature verification passed, and build 2 was installed and launched from Applications. A subsequent independent review verified the live About menu and Command-W closing, then captured representative setup, permission/help, coverage, preparation, active/recovery, release, completion, and interruption states in light and dark appearances, plus an increased-contrast preview with reduced motion and transparency. These checks used an isolated nonblocking simulation. The dedicated screenshot helper produced native window captures after the computer-use capture route failed; no privacy settings were changed. Visible keyboard focus and actual VoiceOver speech remain unverified. No input-engine behavior changed. The ready screen has no persistent footer. Safe development previews retain a small explicit nonblocking label.
