...
The title and toolbar background are hidden while standard window controls remain. Content uses a continuous app surface. Build 2 replaces the scrolling standard About panel with a compact SwiftUI window showing the app icon, version, offline privacy summary, source and Apache 2.0 links, and Drew Levinson credit. Its 340 × 328-point content was checked in light and dark AppKit renders, including native link controls. The optimized build and strict signature verification passed, and build 2 was installed and launched from Applications. Live menu interaction remains unverified for this refinement because native screen capture failed; no new input-engine behavior is involved. The ready screen has no persistent footer. Safe development previews retain a small explicit nonblocking label.
